...
**Generate:** Double-click `tunnelx.exe`, open **Settings**, then click **Generate…** beside **Private key**. The key is created beside the application (`tunnel_key` by default); click **Copy public key** in the dialog. OpenSSH and command-line work are not required.

**Register:** Append the copied public-key line to the server's `authorized_keys`:
...
| Username | Any value (the server checks only the public key) |
...
| `server_user` | Any value. The server authenticates only the public key but records this value in logs. |
