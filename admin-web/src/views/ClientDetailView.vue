<template>
  <section class="page">
    ...
    <div v-if="client" class="detail-stack">
      ...
      <div class="split">
        <article class="panel"><h3>基本资料</h3><dl class="facts"><dt>备注</dt><dd>{{ client.note || '—' }}</dd><dt>最后 IP</dt><dd>{{ client.last_ip || '—' }}</dd><dt>客户端 ID</dt><dd>{{ client.last_client_id || '—' }}</dd><dt>版本</dt><dd>{{ client.last_version || '—' }}</dd><dt>首次接入</dt><dd>{{ formatDate(client.first_seen_at) }}</dd><dt>最后接入</dt><dd>{{ formatDate(client.last_seen_at) }}</dd></dl></article>
